...
const SellComment = ({ comments = [], lookbookId, lookbook, userId }) => {
  ...
  useEffect(() => {
    if (comments.length > 0) {
      const filteredComments = comments
        .filter((comment) => {
          // 판매자의 댓글이거나 현재 사용자가 작성한 댓글은 모두 표시
          return (
            comment.nickname === currentUser ||
            comment.nickname === lookbook.authorNickname
          );
        })
        .map((comment) => ({
          ...comment,
          id: comment.commentId,
          // 댓글의 작성자 또는 판매자이거나 현재 사용자가 작성한 답글만 표시
  ...
  }, [comments, currentUser, lookbook.authorNickname]);
