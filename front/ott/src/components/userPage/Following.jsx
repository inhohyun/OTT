      ...
      <div className="flex flex-col items-center min-h-screen">
        <div className="backdrop-blur-md bg-white bg-opacity-40 text-stone-500 p-6 rounded-2xl max-w-md w-full h-[500px] overflow-y-auto">
          ...
          <div className="flex flex-col space-y-4">
            {Array.from({ length: Math.ceil(visibleFollowers / 2) }, (_, rowIndex) => (
              <div key={rowIndex} className="flex justify-center space-x-4">
                {followers.slice(rowIndex * 2, rowIndex * 2 + 2).map((follower, index) => (
                  <div key={index} className="flex items-center space-x-4 w-1/2 justify-center">
                    <div className="w-12 h-12 bg-gray-300 rounded-full border border-gray-300 overflow-hidden">
                      <img src={follower.image || defaultImage} alt="Follower" className="w-full h-full object-cover" />
                    </div>
