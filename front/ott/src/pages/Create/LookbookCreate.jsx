      ...
      <div className="bg-white bg-opacity-60 p-6 rounded-lg shadow-lg w-full max-w-md">
        <div className="mb-6 flex justify-between items-center">
          <div className="flex items-center">
            <div
              className={`relative w-[50px] h-[24px] rounded-full cursor-pointer transition-colors duration-300 ${isPublic ? 'bg-violet-300' : 'bg-stone-300'}`}
              onClick={() => setIsPublic(!isPublic)}
            >
              <div
                className={`absolute top-0 left-0 w-[22px] h-[22px] bg-white rounded-full shadow-md transition-transform duration-300 ${isPublic ? 'transform translate-x-[26px]' : ''}`}
              ></div>
            </div>
            <span className="ml-3 text-sm">{isPublic ? '공개' : '비공개'}</span>
          </div>
          <button
            className="bg-violet-300 text-white py-2 px-4 rounded-[5px] hover:bg-violet-500 transition"
            onClick={handleSave}
        ...
        <div className="mb-6">
          <div
            className="border-2 border-dashed border-gray-300 w-full h-[300px] mb-4 relative"
            onDrop={handleDrop}
          ...
          <select
            className="w-full border border-gray-300 text-stone-600 rounded-[5px] p-2 mb-2 focus:ring-2 focus:ring-violet-500 focus:outline-none"
            ...
            {selectedCategory &&
              clothesData[selectedCategory].map((item) => (
                <img
                  ...
                  draggable
                  onDragStart={(e) =>
                    handleDragStart(e, { ...item, category: selectedCategory })
                  }
                />
              ))}
          </div>
        ...
        <div className="mb-4">
          <input
            ...
            className="border border-gray-300 rounded-[5px] p-2 w-full focus:ring-2 focus:ring-violet-500 focus:outline-none"
            style={{ fontFamily: 'dohyeon' }}
          />
          <div className="flex flex-wrap mt-2">
            {tags.map((tag, index) => (
              <div
                key={index}
                className="bg-black text-white rounded-[5px] px-3 py-1 text-sm mr-2 mb-2 flex items-center"
              >
                {tag}
                <button
