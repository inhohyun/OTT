...
const Recommend = () => {
  const lookbooksData = {
    '키·몸무게': Array.from({ length: 15 }, (_, idx) => ({
      id: idx + 1,
      attributes: {
        height: 180,
        weight: 75,
    ...
    '선호 스타일': Array.from({ length: 12 }, (_, idx) => ({
      id: idx + 1,
      attributes: {
        height: 170,
        weight: 70,
  ...
  const [visibleLookbooks, setVisibleLookbooks] = useState(
    Object.keys(lookbooksData).reduce(
      (acc, key) => ({ ...acc, [key]: initialLimit }),
      {}
    )
  );
